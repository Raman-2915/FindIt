...
export default function FoundItems() {
  ...
  async function loadCategories() {
    try {
      ...
      // Categories API is currently not available in the backend.
      // The proper API will be added during the backend improvement phase.
      setCategories([]);
    }
  }
  ...
  async function loadItems() {
    try {
      ...
      const response = await api.get<FoundItemsResponse>("/found-items", {
        params: {
          page,
          limit: 12,
      ...
      setError(
        err.response?.data?.message ||
          "Unable to load found items. Please try again.",
      );
        ...
        <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-500 sm:text-base">
              See items that members of the FindIt community have found and
              reported.
            </p>
          </div>
      ...
      <section className="mb-7 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
        ...
        {showFilters && (
          <div className="mt-4 rounded-xl bg-slate-50 p-4 text-xs text-slate-500 lg:hidden">
            Use the search field and category selector above to narrow down
            found item reports.
          </div>
        )}
      </section>
      ...
      {!loading && !error && items.length === 0 && (
        <div className="rounded-2xl border border-dashed border-slate-300 bg-white px-6 py-14 text-center">
          ...
          <p className="mx-auto mt-2 max-w-md text-sm leading-6 text-slate-400">
            There are no found item reports matching your current filters.
          </p>
      ...
      {!loading && !error && items.length > 0 && (
        <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {items.map((item) => (
            ...
            >
              {/* Card visual */}
              <div className="relative flex h-36 items-center justify-center bg-gradient-to-br from-emerald-50 via-teal-50 to-slate-50">
                <FolderSearch
                  size={42}
                  strokeWidth={1.5}
                  className="text-emerald-500 transition duration-200 group-hover:scale-110"
                />

                <span className="absolute right-4 top-4 rounded-full bg-white/90 px-2.5 py-1 text-[10px] font-bold uppercase tracking-wide text-emerald-600 shadow-sm">
                  {item.status}
                </span>
              </div>
